...
static double ready_delay_timer = 0.0;
...
void resetGame(Pacman* pacman){
    ...
    setGhostReleaseInterval(5.0 - (current_stage - 1) * 0.5); // 5초에서 점점 빨라짐
    if (getGhostReleaseInterval() < 2.0) setGhostReleaseInterval(2.0); // 최소 2.0 초 유지
...
void handleLogic(Pacman* pacman) {
    switch(current_state) {
        ...
        case STATE_PLAYING:
            ...
            if (!bonus_fruit.active) {
                if (getCookiesEaten() == BONUS_FRUIT_TRIGGER_COOKIES || 
                    (getCookiesEaten() == getTotalCookies() / 2)) {
    ...
    while (attempts < max_attempts) {
        ...
        if (tile == EMPTY) {
            ...
            if (!position_occupied) {
                ...
                bonus_fruit.timer = FRUIT_MIN_DURATION + (FRUIT_MAX_DURATION - FRUIT_MIN_DURATION) * (rand() / (double)RAND_MAX); // 8~15초 (실수값)
                bonus_fruit.score = getFruitScore(bonus_fruit.type);
                
                
                debug_log("Bonus fruit spawned at (%d, %d), total_cookies: %d\n", 
                         random_x, random_y, getTotalCookies());
                break;
            }
        }
        attempts++;
